...
#COMENTARIO SOLO INÉS: En base a los comentarios de la profesora de SQL, intento importar el csv con la siguiente fórmula:
LOAD DATA INFILE 
'/Users/inesbenitodiaz/mysql-files/CVS SQL/duracion.csv'
INTO TABLE duracion 
...
#Me sale el Error Code: 1290. Tengo que dar acceso a la carpeta donde se aloja el csv para poder importarlo.
#Para ello, veo qué acceso tiene actualmente MYSQL dentro de mi ordenador:

SELECT @@secure_file_priv;
#La respuesta es NULL, por lo que MYSQL tiene acceso a todas las carpetas, pero aún así no lo lee. 

#Pruebo con LOCAL, primero comprobando que esté ON para poder usarlo:
SHOW GLOBAL VARIABLES LIKE 'local_infile'; # Está en ON
...
LINES TERMINATED BY '\r\n' 
IGNORE 1 ROWS;
#Me saca el error 2068, al parecer (según he investigado) MySQL lo bloquea en Mac por seguridad. De tal manera que solo me queda exportar
#como hice en un incio, descrito abajo.
...
SELECT DATE(comienzo_atraccion_fecha_hora) AS fecha,
		COUNT(t_id) AS total_visitas #no hay DISTINCT porque el t_id es un identificador único.
FROM ticket_atracciones
GROUP BY fecha
...
CREATE VIEW ranking_visitas_desc AS
SELECT 
	id_visitante,
...
SELECT
	recuento_atracciones AS nº_atracciones_subidas,
...
CREATE VIEW media_valoracion_procedencia AS
SELECT
	id_visitante,
    procedencia,
    media_valoraciones,
...
) AS media_valoracion_procedencia;

SELECT id_visitante, procedencia, media_valoraciones
FROM media_valoracion_procedencia
...
SELECT
    ...
    ROUND(COUNT(DISTINCT id_visitante) * 100.0 /(SELECT COUNT(DISTINCT id_visitante) FROM media_valoracion_procedencia),2) AS porcentaje_visitantes
FROM media_valoracion_procedencia
WHERE ranking_valoraciones <= 3
GROUP BY procedencia
ORDER BY total_visitantes DESC;

#Por otro lado, vamos a ver la cantidad de visitantes, y su respectivo porcentaje, agrupando por cada una de las 3 peores valoraciones:
SELECT 
	media_valoraciones,
	COUNT(DISTINCT id_visitante) AS total_visitantes,
    ROUND(COUNT(DISTINCT id_visitante)*100.0 / (SELECT COUNT(DISTINCT id_visitante) FROM media_valoracion_procedencia),2) AS porcentaje_visitantes
...
#Hay 7 visitantes que cuyos tiempos de espera son incongruentes, ya que su duración es menor al tiempo de espera. Obviaremos estos visitantes
#cuando ordenemos de menor a mayor tiempo de espera:
SELECT 
    t.id_visitante,
    SUM(d.duracion - t.tiempo_de_espera) AS tiempo_total_sin_esperas
FROM ticket_atracciones t
INNER JOIN duracion d ON t.t_id = d.t_id
WHERE d.duracion > t.tiempo_de_espera
GROUP BY t.id_visitante
ORDER BY tiempo_total_sin_esperas ASC;
...
CREATE VIEW ranking_valoracion_atracciones_desc AS
SELECT
	t.atraccion,
...
SELECT
    ...
    tiempo_total_espera,
    media_valoracion
FROM ranking_valoracion_atracciones_desc
WHERE ranking IN (1, 2, 3)
ORDER BY ranking ASC;

#Tres atracciones peor valoradas
CREATE VIEW ranking_valoracion_atracciones_asc AS
SELECT
	t.atraccion,
	SUM(t.tiempo_de_espera) AS tiempo_total_espera,
	AVG(v.valoracion) AS media_valoracion,
...
#Como vemos que la primera opción sale "desconocido", vamos a ver los datos de la segunda opción, que sería la primera opción con nombre real:
SELECT
	atraccion,
    recuento_subidas_por_atraccion
FROM subidas_por_atraccion
...
#Creo una vista para las entradas correctas de pase rápido:
CREATE VIEW visitas_atraccion_pase_rapido_correctas AS
SELECT 
	atraccion,
...
#Creo una vista para las entradas tanto correctas como incorrectas de pase rápido:
CREATE VIEW visitas_atraccion_pase_rapido_incorrectas AS
SELECT 
	atraccion,
	COUNT(t_id) AS recuento_visitas,
	RANK() OVER(ORDER BY COUNT(t_id) DESC) AS ranking_visitantes
...
SELECT
	 atraccion,
     recuento_visitas
FROM visitas_atraccion_pase_rapido_correctas
...
SELECT
	 atraccion,
     recuento_visitas
FROM visitas_atraccion_pase_rapido_incorrectas
WHERE recuento_visitas = (
	SELECT MAX(recuento_visitas)
    FROM visitas_atraccion_pase_rapido_incorrectas
);
...
#Ordenamos las duraciones de menor a mayor
SELECT
	d.duracion,
	ROUND(AVG(v.valoracion), 2) AS media_valoraciones
FROM duracion d
...
#Ordenamos las duraciones de mayor a menor
SELECT
	d.duracion,
	ROUND(AVG(v.valoracion), 2) AS media_valoraciones
FROM duracion d
...
#Los visitantes que tienen mayores duraciones, tienen una valoracion media más constante, entre 5-3. 
#Mientras que los visitantes que tienen duraciones más bajas, tienen valoraciones muy variables.
...
#21) Relación tiempo de espera y valoración: ¿Cuánto más tiempo de espera, menos valoración? 
SELECT
	t.tiempo_de_espera,
    ROUND(AVG(v.valoracion),2) AS media_valoraciones
FROM ticket_atracciones t
...
# 22) Calcular la media de valoraciones
SELECT ROUND(AVG(valoracion),2) AS media_valoraciones
FROM valoraciones_emociones;
#Resultado: 4,98

#23) Calcular la media del tiempo de espera por atracción
SELECT 
	atraccion,
	ROUND(AVG(tiempo_de_espera),2) AS tiempo_de_espera
FROM ticket_atracciones
GROUP BY atraccion
ORDER BY tiempo_de_espera DESC;

# 24) Recaudación y número de visitantes por día 
SELECT
    DATE(comienzo_atraccion_fecha_hora) AS dia,
    SUM(coste) AS coste,
    COUNT(DISTINCT id_visitante) AS numero_visitantes
FROM ticket_atracciones
GROUP BY dia
ORDER BY dia ASC;

# 25) Porcentaje de visitantes por procedencia
SELECT 
	procedencia,
	COUNT(DISTINCT id_visitante) AS numero_visitantes,
    ROUND(COUNT(DISTINCT id_visitante)*100.0 / (SELECT COUNT(DISTINCT id_visitante) FROM procedencia),2) AS porcentaje
FROM procedencia
GROUP BY procedencia
ORDER BY numero_visitantes DESC;
#La mayor parte de los visitantes son españoles: 19,32%
